...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("ZST")
    updateDisplayUnit();
}
...
{
    QString ret = QString("zestcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ZST unit input in lnReqAmount, we generate the URI with ZST as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::ZST, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
